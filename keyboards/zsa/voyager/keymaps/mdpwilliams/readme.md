...
- **Layer 0 (`_BASE`)** – standard QWERTY with the home row mods above.
  Top-left corner is `Backspace`, outer-pinky home-row key is `Esc`
  (swapped from stock QMK's usual placement). The old Caps Word key is
  currently unassigned (`KC_NO`) — a placeholder until it's replaced
  with something else. Left inner thumb key is `Enter` (tap) / layer 1
  (hold). Right inner thumb key is `Space` (tap) / layer 2 (hold).
